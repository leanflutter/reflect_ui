...
import 'package:reflect_ui/src/widgets/slider/slider_thumb_painter.dart';
...
const double _kSliderHeight = 2.0 * (SliderThumbPainter.radius + _kPadding);
...
class _RenderSlider extends RenderConstrainedBox
  ...
  double get _thumbCenter {
    ...
    return lerpDouble(_trackLeft + SliderThumbPainter.radius,
        _trackRight - SliderThumbPainter.radius, visualPosition)!;
  }
  ...
  void _handleDragUpdate(DragUpdateDetails details) {
    if (isInteractive) {
      final double extent = math.max(_kPadding,
          size.width - 2.0 * (_kPadding + SliderThumbPainter.radius));
  ...
  bool hitTestSelf(Offset position) {
    return (position.dx - _thumbCenter).abs() <
        SliderThumbPainter.radius + _kPadding;
  }
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    if (visualPosition > 0.0) {
      ...
      canvas.drawRRect(
          RRect.fromLTRBXY(trackLeft, trackTop, trackActive, trackBottom, 2, 2),
          paint);
    }
    ...
    if (visualPosition < 1.0) {
      ...
      canvas.drawRRect(
          RRect.fromLTRBXY(
              trackActive, trackTop, trackRight, trackBottom, 2, 2),
          paint);
    }

    final Offset thumbCenter = Offset(trackActive, trackCenter);
    SliderThumbPainter(color: thumbColor).paint(
        canvas,
        Rect.fromCircle(
            center: thumbCenter, radius: SliderThumbPainter.radius));
  }
