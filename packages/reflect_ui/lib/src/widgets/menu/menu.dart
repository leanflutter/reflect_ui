...
/// The type of builder function used by [Menu.builder] to build the
/// widget that the [Menu] surrounds.
///
...
/// the menu with.
///
/// The `child` is an optional child supplied as the [Menu.child]
/// attribute. The child is intended to be incorporated in the result of the
/// function.
typedef MenuTargetBuilder = Widget Function(
...
///
/// When creating a menu with [MenuBar] or a [SubmenuButton], a [Menu] is
/// not needed, since they provide their own internally.
///
/// The [Menu] is meant to be a slightly lower level interface than
/// [MenuBar], used in situations where a [MenuBar] isn't appropriate, or to
/// construct widgets or screen regions that have submenus.
///
/// {@tool dartpad}
/// This example shows how to use a [Menu] to wrap a button and open a
...
///
/// {@tool dartpad}
/// This example shows how to use a [Menu] to create a cascading context
...
/// that clicks on the [Menu] area will cause the menus to be closed.
///
/// ** See code in examples/api/lib/material/menu_anchor/menu_anchor.1.dart **
/// {@end-tool}
///
/// {@tool dartpad}
/// This example demonstrates a simplified cascading menu using the [Menu]
/// widget.
///
/// ** See code in examples/api/lib/material/menu_anchor/menu_anchor.3.dart **
/// {@end-tool}
class Menu extends StatefulWidget {
  /// Creates a const [Menu].
  ///
  ...
  /// The [targetFocusNode] attribute is the optional [FocusNode] also associated
...
class _MenuState extends State<Menu> {
  ...
  // view's edges.
  final GlobalKey<_MenuState> _anchorKey = GlobalKey<_MenuState>(
    debugLabel: kReleaseMode ? null : 'Menu',
  );
  _MenuState? _parent;
  late final FocusScopeNode _menuScopeNode;
  MenuController? _internalMenuController;
  final List<_MenuState> _anchorChildren = <_MenuState>[];
  ScrollPosition? _scrollPosition;
  Size? _viewSize;
  final OverlayPortalController _overlayController = OverlayPortalController(
    debugLabel: kReleaseMode ? null : 'Menu controller',
  );
  ...
  void didChangeDependencies() {
    ...
    final _MenuState? newParent = _MenuState._maybeOf(context);
      ...
      child: Builder(
        ...
        builder: (BuildContext context) {
          return widget.targetBuilder
                  ?.call(context, _menuController, widget.target) ??
              widget.target ??
              const SizedBox();
        },
      ),
  ...
  void _addChild(_MenuState child) {
  ...
  void _removeChild(_MenuState child) {
  ...
  List<_MenuState> _getFocusableChildren() {
    if (_parent == null) {
      return <_MenuState>[];
    }
    return _parent!._anchorChildren.where(
      (_MenuState menu) {
        return menu.widget.targetFocusNode?.canRequestFocus ?? false;
      },
    ).toList();
  }

  _MenuState? get _nextFocusableSibling {
    final List<_MenuState> focusable = _getFocusableChildren();
  ...
  _MenuState? get _previousFocusableSibling {
    final List<_MenuState> focusable = _getFocusableChildren();
    if (focusable.isEmpty) {
      return null;
    }
  ...
  _MenuState get _root {
    _MenuState anchor = this;
  ...
  _MenuState get _topLevel {
    _MenuState handle = this;
  ...
  void _focusButton() {
    if (widget.targetFocusNode == null) {
      return;
    }
    assert(_debugMenuInfo('Requesting focus for ${widget.targetFocusNode}'));
  ...
  /// Open the menu, optionally at a position relative to the [Menu].
  ///
  ...
  /// the local coordinates of the [Menu], ignoring any
  /// [MenuStyle.alignment] and/or [Menu.alignmentOffset] that were
  /// specified.
  ...
  void _closeChildren({bool inDispose = false}) {
    ...
    for (final _MenuState child in List<_MenuState>.from(_anchorChildren)) {
  ...
  // changes.
  static _MenuState? _maybeOf(BuildContext context) {
    return context.dependOnInheritedWidgetOfExactType<_MenuScope>()?.anchor;
  }
...
/// A controller to manage a menu created by a [MenuBar] or [Menu].
///
...
/// See also:
///
/// * [Menu], a widget that defines a region that has submenu.
...
class MenuController {
  ...
  /// [Menu]. A [MenuController] is also be received by the
  /// [Menu.builder] when invoked.
  ///
  /// If the menu's anchor point (either a [MenuBar] or a [Menu]) is
  ...
  /// the coordinate space of the [Menu] this controller is attached to.
  ///
  /// If given, the `position` will override the [Menu.alignmentOffset]
  /// given to the [Menu].
  ///
  /// If the menu's anchor point (either a [MenuBar] or a [Menu]) is
  /// scrolled by an ancestor, or the view changes size, then any open menu will
  ...
  void _attach(_MenuState anchor) {
    _anchor = anchor;
  }

  void _detach(_MenuState anchor) {
    if (_anchor == anchor) {
      _anchor = null;
    }
  }
...
/// See also:
///
/// * [Menu], a widget that creates a region with a submenu and shows it
///   when requested.
...
/// A button for use in a [MenuBar], in a menu created with [Menu], or on
...
class MenuItemButton extends StatefulWidget {
  ...
  ///
  /// If the menu item button is a descendent of [Menu] or [MenuBar], then
...
class _MenuItemButtonState extends State<MenuItemButton> {
  ...
  _MenuState? get _anchor => _MenuState._maybeOf(context);
  ...
  void _handleFocusChange() {
    if (!_focusNode.hasPrimaryFocus) {
      ...
      _MenuState._maybeOf(context)?._closeChildren();
    }
  }
  ...
  void _handleSelect() {
    ...
    if (widget.closeOnActivate) {
      _MenuState._maybeOf(context)?._root._close();
    }
...
/// - [Menu], a widget that defines a region which can host a cascading
///   menu.
...
/// [children].
///
/// If [children] is empty, then this menu item will appear disabled.
///
/// See also:
///
...
class SubmenuButton extends StatefulWidget {
  ...
  ///
  /// The [child] and [children] attributes are required.
  const SubmenuButton({
    super.key,
    this.onHover,
    this.onFocusChange,
  ...
  ///
  /// If [children] is empty, then the button for this menu item will be
  /// disabled.
  final List<Widget> children;
...
class _SubmenuButtonState extends State<SubmenuButton> {
  ...
  EdgeInsets _computeMenuPadding(BuildContext context) {
    final WidgetStateProperty<EdgeInsetsGeometry?> insets =
        widget.menuStyle?.padding ?? _MenuDefaultsM3(context).padding!;
    return insets
...
/// See also:
///
///  * [Menu], a widget that hosts a cascading submenu.
...
/// MenuBar-specific private specialization of [Menu] so that it can act
/// differently in regards to orientation, how open works, and what gets built.
class _MenuBarAnchor extends Menu {
  ...
  @override
  State<Menu> createState() => _MenuBarAnchorState();
}

class _MenuBarAnchorState extends _MenuState {
  @override
  bool get _isOpen {
    // If it's a bar, then it's "open" if any of its children are open.
    for (final _MenuState child in _anchorChildren) {
      if (child._isOpen) {
        return true;
      }
    }
...
class _MenuPreviousFocusAction extends PreviousFocusAction {
  ...
  bool invoke(PreviousFocusIntent intent) {
    ...
    final _MenuState? anchor = _MenuState._maybeOf(context);
  ...
  static bool _moveToPreviousFocusable(_MenuState currentMenu) {
    final _MenuState? sibling = currentMenu._previousFocusableSibling;
...
class _MenuNextFocusAction extends NextFocusAction {
  ...
  static bool _moveToNextFocusable(_MenuState currentMenu) {
    final _MenuState? sibling = currentMenu._nextFocusableSibling;
    sibling?._focusButton();
    return true;
  }
}
...
class _MenuDirectionalFocusAction extends DirectionalFocusAction {
  ...
  void invoke(DirectionalFocusIntent intent) {
    ...
    final bool buttonIsFocused =
        anchor.widget.targetFocusNode?.hasPrimaryFocus ?? false;
    ...
    assert(
      _debugMenuInfo(
          'In _MenuDirectionalFocusAction, current node is ${anchor.widget.targetFocusNode?.debugLabel}, '
          'button is${buttonIsFocused ? '' : ' not'} focused. Assuming ${orientation.name} orientation.'),
    );

    final bool Function(_MenuState) traversal =
  ...
  bool _moveToNext(_MenuState currentMenu) {
    ...
    // correct node.
    if (currentMenu.widget.targetFocusNode != null) {
      if (currentMenu.widget.targetFocusNode!.nearestScope != null) {
        final FocusTraversalPolicy? policy =
            FocusTraversalGroup.maybeOf(primaryFocus!.context!);
        policy?.invalidateScopeData(
          currentMenu.widget.targetFocusNode!.nearestScope!,
        );
      }
    }
    return false;
  ...
  bool _moveToNextFocusableTopLevel(_MenuState currentMenu) {
    final _MenuState? sibling = currentMenu._topLevel._nextFocusableSibling;
    sibling?._focusButton();
    return true;
  }

  bool _moveToParent(_MenuState currentMenu) {
    assert(_debugMenuInfo('Moving focus to parent menu button'));
    if (!(currentMenu.widget.targetFocusNode?.hasPrimaryFocus ?? true)) {
      currentMenu._focusButton();
    }
    return true;
  }

  bool _moveToPrevious(_MenuState currentMenu) {
  ...
  bool _moveToPreviousFocusableTopLevel(_MenuState currentMenu) {
    final _MenuState? sibling = currentMenu._topLevel._previousFocusableSibling;
    sibling?._focusButton();
    return true;
  }

  bool _moveToSubmenu(_MenuState currentMenu) {
...
/// [Menu] or [MenuBar]. Provided menu items such as [MenuItemButton] and
...
class _MenuAcceleratorLabelState extends State<MenuAcceleratorLabel> {
  ...
  void didChangeDependencies() {
    ...
    _anchor = _MenuState._maybeOf(context);
  ...
  Offset getPositionForChild(Size size, Size childSize) {
    ...
    if (childSize.height >= allowedRect.height) {
      ...
      if (offTop(y)) {
        ...
        if (!offTop(newY)) {
          // Only move the menu up if its parent is horizontal (Menu/MenuBar).
...
class _MenuPanelState extends State<_MenuPanel> {
  ...
  Widget build(BuildContext context) {
    final (MenuStyle? themeStyle, MenuStyle defaultStyle) =
      ...
      Axis.vertical => (null, _MenuDefaultsM3(context)),
    };
...
class _Submenu extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final (MenuStyle? themeStyle, MenuStyle defaultStyle) =
      ...
      Axis.horizontal || null => (null, _MenuBarDefaultsM3(context)),
      Axis.vertical => (null, _MenuDefaultsM3(context)),
    };
    T? effectiveValue<T>(T? Function(MenuStyle? style) getProperty) {
